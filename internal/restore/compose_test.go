package restore_test

import (
	"context"
	"fmt"
	"io"
	"testing"

	"github.com/offen/restore-manager/internal/docker"
	"github.com/offen/restore-manager/internal/restore"
	"github.com/offen/restore-manager/internal/sse"
)

// failingBackend wraps mockBackend but fails on a specific key.
type failingBackend struct {
	mockBackend
	failOnKey string
}

func (m *failingBackend) Download(ctx context.Context, key string, w io.Writer) error {
	if key == m.failOnKey {
		return fmt.Errorf("simulated download failure for %s", key)
	}
	w.Write([]byte("data"))
	return nil
}

func TestComposeNewVolumeSuccess(t *testing.T) {
	b := sse.NewBroadcaster()
	dc := &mockDockerClient{}
	backend := &mockBackend{data: []byte("backup-data")}

	orch := restore.NewOrchestrator(dc, b)
	orch.SetStagingDir(t.TempDir())

	ch := b.Subscribe("compose-nv-1")
	defer b.Unsubscribe("compose-nv-1", ch)

	req := restore.ComposeRestoreRequest{
		Token:   "compose-nv-1",
		Project: "myproject",
		Mode:    restore.ModeNewVolume,
		Volumes: []restore.VolumeRestore{
			{
				VolumeName: "pgdata",
				BackupKey:  "backup-pgdata.tar.gz",
				TargetName: "pgdata_restored",
				BackupSize: int64(len(backend.data)),
			},
			{
				VolumeName: "redisdata",
				BackupKey:  "backup-redisdata.tar.gz",
				TargetName: "redisdata_restored",
				BackupSize: int64(len(backend.data)),
			},
		},
	}

	err := orch.RunCompose(context.Background(), req, backend)
	if err != nil {
		t.Fatalf("RunCompose: %v", err)
	}

	// Both volumes should be created.
	if len(dc.createdVolumes) != 2 {
		t.Fatalf("created volumes: got %v, want 2", dc.createdVolumes)
	}
	if dc.createdVolumes[0] != "pgdata_restored" {
		t.Fatalf("first volume: got %q, want %q", dc.createdVolumes[0], "pgdata_restored")
	}
	if dc.createdVolumes[1] != "redisdata_restored" {
		t.Fatalf("second volume: got %q, want %q", dc.createdVolumes[1], "redisdata_restored")
	}

	events := drainEvents(ch)

	// Events should have volume_total=2.
	foundVolumeTotal := false
	for _, ev := range events {
		if ev.VolumeTotal == 2 {
			foundVolumeTotal = true
			break
		}
	}
	if !foundVolumeTotal {
		t.Fatalf("no event with volume_total=2 found: %+v", events)
	}

	// "complete" step should be emitted.
	assertHasStep(t, events, "complete")
}

func TestComposeFullStackSuccess(t *testing.T) {
	b := sse.NewBroadcaster()
	dc := &fullStackMockDocker{
		containers: []docker.ContainerInfo{
			{ID: "c1", Name: "proj-postgres-1", Service: "postgres", State: "running"},
			{ID: "c2", Name: "proj-app-1", Service: "app", State: "running"},
		},
	}
	backend := &mockBackend{data: []byte("backup-data")}

	orch := restore.NewOrchestrator(dc, b)
	orch.SetStagingDir(t.TempDir())

	ch := b.Subscribe("compose-fs-1")
	defer b.Unsubscribe("compose-fs-1", ch)

	req := restore.ComposeRestoreRequest{
		Token:    "compose-fs-1",
		Project:  "proj",
		Services: []string{"postgres", "app"},
		DependsOn: map[string][]string{
			"app": {"postgres"},
		},
		Mode: restore.ModeFullStack,
		Volumes: []restore.VolumeRestore{
			{
				VolumeName: "pgdata",
				BackupKey:  "backup-pgdata.tar.gz",
				BackupSize: int64(len(backend.data)),
			},
			{
				VolumeName: "redisdata",
				BackupKey:  "backup-redisdata.tar.gz",
				BackupSize: int64(len(backend.data)),
			},
		},
	}

	err := orch.RunCompose(context.Background(), req, backend)
	if err != nil {
		t.Fatalf("RunCompose: %v", err)
	}

	// Containers should have been stopped and restarted.
	if len(dc.stoppedIDs) == 0 {
		t.Fatal("expected containers to be stopped")
	}
	if len(dc.startedIDs) == 0 {
		t.Fatal("expected containers to be restarted")
	}

	// Both volumes should be processed (2 oneshot runs).
	if len(dc.oneshotConfigs) != 2 {
		t.Fatalf("oneshot runs: got %d, want 2", len(dc.oneshotConfigs))
	}

	events := drainEvents(ch)
	assertHasStep(t, events, "stopping_containers")
	assertHasStep(t, events, "starting_containers")
	assertHasStep(t, events, "complete")
}

func TestComposeFullStackPartialFailureRestartsContainers(t *testing.T) {
	b := sse.NewBroadcaster()
	dc := &fullStackMockDocker{
		containers: []docker.ContainerInfo{
			{ID: "c1", Name: "proj-postgres-1", Service: "postgres", State: "running"},
		},
	}
	backend := &failingBackend{
		mockBackend: mockBackend{data: []byte("data")},
		failOnKey:   "backup-redisdata.tar.gz",
	}

	orch := restore.NewOrchestrator(dc, b)
	orch.SetStagingDir(t.TempDir())

	ch := b.Subscribe("compose-fs-fail-1")
	defer b.Unsubscribe("compose-fs-fail-1", ch)

	req := restore.ComposeRestoreRequest{
		Token:    "compose-fs-fail-1",
		Project:  "proj",
		Services: []string{"postgres"},
		Mode:     restore.ModeFullStack,
		Volumes: []restore.VolumeRestore{
			{
				VolumeName: "pgdata",
				BackupKey:  "backup-pgdata.tar.gz",
				BackupSize: 4,
			},
			{
				VolumeName: "redisdata",
				BackupKey:  "backup-redisdata.tar.gz",
				BackupSize: 4,
			},
		},
	}

	err := orch.RunCompose(context.Background(), req, backend)
	// Should return an error since the second volume failed.
	if err == nil {
		t.Fatal("expected error for partial volume failure")
	}

	// Containers MUST still be restarted despite the failure.
	if len(dc.startedIDs) == 0 {
		t.Fatal("expected containers to be restarted even after failure")
	}

	events := drainEvents(ch)
	assertHasStep(t, events, "failed")
}
